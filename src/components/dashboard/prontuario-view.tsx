...
export function ProntuarioView({ pacienteId }: { pacienteId: string }) {
  const prontuario = getProntuarioById(pacienteId);
  const tarefas = getTarefasByPacienteId(pacienteId);
  const resumoGeral = getResumoGeral(pacienteId);
  ...
  const humorData = registrosHumor.map((r) => ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
        ...
        <span
          className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-medium self-start sm:self-center ${
              ...
              <div className="mt-4 h-56">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={humorData}>
                    ...
                    <Area type="monotone" dataKey="valor" stroke="var(--color-primary)" fill="url(#humorGrad)" strokeWidth={2} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
                ...
                <div className="space-y-4 min-w-0">
                  ...
                  {sessaoAtual.conclusoesIA ? (
                    <div className="rounded-xl border bg-card p-5 shadow-sm">
                      ...
                      {sessaoAtual.insights.length > 0 && (
                        <div className="mt-5">
                          <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">
                            Insights extraídos
                          </h4>
                          <div className="flex flex-wrap gap-2">
                            {sessaoAtual.insights.map((i, idx) => (
                              <span
                                key={idx}
                  ...
                  {/* Transcrição */}
                  {sessaoAtual.transcricao && (
                    <div className="rounded-xl border bg-card p-5 shadow-sm">
                      <h3 className="text-sm font-semibold text-foreground flex items-center gap-2">
                        <FileText className="h-4 w-4 text-primary" />
                        Transcrição da Sessão
                      </h3>
                      <pre className="mt-3 max-h-96 overflow-auto whitespace-pre-wrap rounded-lg bg-muted/50 p-4 text-xs font-mono text-foreground leading-relaxed border">
                        {sessaoAtual.transcricao}
                      </pre>
                    </div>
                  )}
...
function ChatWhatsApp({
  ...
  return (
    <div className="rounded-xl border bg-card shadow-sm overflow-hidden flex flex-col h-[640px]">
      {/* Header */}
...
function SinteseClinica({
  ...
  pacienteId: string;
  resumoGeral: NonNullable<ReturnType<typeof getResumoGeral>>;
}) {
...
function ProfissionalAssistenteTexto({
  ...
  return (
    <Tabs defaultValue="profissional" className="mt-3">
      <TabsList>
        <TabsTrigger value="profissional">Profissional</TabsTrigger>
        <TabsTrigger value="assistente">Assistente de IA</TabsTrigger>
      </TabsList>
...
function ConclusoesProfAssistente({
  ...
  storageKey: string;
  conclusoesIA: NonNullable<
    ReturnType<typeof getProntuarioById>
  >["sessoes"][number]["conclusoesIA"];
  abordagem: Abordagem;
